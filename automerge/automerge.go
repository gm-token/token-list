...
func (m *Automerger) ProcessPR(ctx context.Context, pr *github.PullRequest) error {
	...
	if err := m.commitTokenDiff(tt, pr); err != nil {
		return fmt.Errorf("failed to commit token diff: %v", err)
	}
...
func (m *Automerger) commitTokenDiff(tt []Token, pr *github.PullRequest) error {
...
func (m *Automerger) processTokenlist(ctx context.Context, d *diff.FileDiff, assets []string) ([]Token, error) {
	// log assets
	klog.V(1).Infof("found %d image assets", len(assets))
	...
	for _, h := range d.Hunks {
		...
		for _, t := range tt {
			...
			if err := verifyLogoURI(t.LogoURI, t.Address); err != nil {
...
func verifyLogoURI(uri string, address string) error {
	// TODO: verify match
	if strings.HasPrefix(uri, "https://raw.githubusercontent.com/solana-labs/token-list/main/assets/mainnet/"+address+"/") {
		return nil
	}
